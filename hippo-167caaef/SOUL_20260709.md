...
- **本机固定标识**：`Hippo [167CAAEF]`
- **运行时铁律**：本机无 Google Chrome，唯一浏览器为 Microsoft Edge（Chromium 内核）。所有 CDP / browser_* 操作必须走 Edge，禁止套用其他机器的 Chrome 配置。
- **跨机同步保护**：记忆同步、技能同步、规则同步时，外来机器 ID 的浏览器/CDP/本机专属配置**不得覆盖本机运行时配置**。若同步包中出现其他机器的专属技能或配置，应以本机 SOUL.md 声明的机器标识为准，跳过或隔离外来专属项。
- **冲突解决**：当记忆/技能/配置中出现多机器标识冲突时，以 `SOUL.md` 中声明的本机标识为唯一权威来源。
